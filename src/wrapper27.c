...
py_bottom_clear_groups(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "Oii", &data, &num_layers, &has_garbage))
  {
    return NULL;
  }
  int did_clear = bottom_clear_groups_and_garbage((puyos_t*)data->ob_bytes, num_layers, !!has_garbage);
...
py_bottom_resolve(PyObject *self, PyObject *args)
{
  ...
  int chain = bottom_resolve((puyos_t*)data->ob_bytes, num_layers, !!has_garbage);
...
py_tall_clear_groups(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "Oiiii", &data, &num_layers, &chain_number, &tsu_rules, &has_garbage))
  {
    return NULL;
  }
  int score = tall_clear_groups_and_garbage((puyos_t*)data->ob_bytes, num_layers, chain_number, !!tsu_rules, !!has_garbage);
...
py_tall_resolve(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "Oiii", &data, &num_layers, &tsu_rules, &has_garbage))
  {
    return NULL;
  }

  int chain;
  int score = tall_resolve((puyos_t*)data->ob_bytes, num_layers, !!tsu_rules, !!has_garbage, &chain);
